...
def test_handle_action_bad_state():
    with pytest.raises(ValueError):
        mqry.handle_action(mqry.TEST_ID, gen_random_not_valid_str(),
                           mqry.TEST_ACTION,
...
def test_handle_action_bad_action():
    with pytest.raises(ValueError):
        mqry.handle_action(mqry.TEST_ID, mqry.TEST_STATE,
...
def test_handle_action_valid_return():
    for state in mqry.get_states():
        for action in mqry.get_valid_actions_by_state(state):
            print(f'{action=}')
            new_state = mqry.handle_action(mqry.TEST_ID, state, action,
                                           manu=mqry.SAMPLE_MANU,
                                           referee='Some ref')
            print(f'{new_state=}')
...
#                               action=action,
#                               manu=mqry.SAMPLE_MANU,
#                               referee="Some ref") == expected_state
#
#
...
#                                   action=action,
#                                   manu=mqry.SAMPLE_MANU,
#                                   referee="Some ref") == mqry.IN_REF_REV
#
#
# @pytest.mark.parametrize(
...
#                                   referee="Mock ref") == mqry.IN_REF_REV
#
#
